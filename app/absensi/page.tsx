...
type AttendanceStatus = "Hadir" | "Tidak Masuk" | "Izin" | "Sakit" | "Alpa";
...
function buildRows(students: StudentApiRow[], session: AttendanceSessionApi | undefined, selectedDate: string): AttendanceRow[] {
  ...
  return students
    ...
    .map((student) => {
      ...
      return {
        ...
        status: record ? (attendanceStatusLabel(record.status) as AttendanceStatus) : "Tidak Masuk"
      };
    })
...
export default function AttendancePage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const [attendanceResponse, studentsResponse] = await Promise.all([fetch("/api/attendance"), fetch("/api/students")]);
      if (!attendanceResponse.ok || !studentsResponse.ok) {
        setLoading(false);
        setMessage("Gagal memuat sinkronisasi data murid.");
        return;
      }

      const [attendancePayload, studentsPayload] = (await Promise.all([attendanceResponse.json(), studentsResponse.json()])) as [
        { data?: AttendanceSessionApi[] },
        { data?: StudentApiRow[] }
      ];
      const nextSessions = attendancePayload.data ?? [];
      const nextStudents = studentsPayload.data ?? [];
      const nextDate = nextSessions[0]?.date ? dateKey(nextSessions[0].date) : isoToday();
  ...
  useEffect(() => {
    ...
    setRows(buildRows(students, session, selectedDate));
