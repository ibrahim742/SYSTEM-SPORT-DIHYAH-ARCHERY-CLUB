...
type StudentOption = { id: string; name: string; clubName: string; sportId: string; sportName: string; levelLabel: string; progress: number };
const PAGE_SIZE = 5;
...
export function AssignProgramForm({ programs, students, sports }: { programs: ProgramOption[]; students: StudentOption[]; sports: SportOption[] }) {
  ...
  const filteredStudents = useMemo(() => students.filter((student) => !sportId || student.sportId === sportId), [students, sportId]);
  ...
  async function submit() {
    ...
    if (failed) {
      const error = await failed.json().catch(() => ({ error: "Gagal assign program" }));
      setMessage(error.error ?? "Gagal assign program");
      return;
    }

    setMessage(`${studentIds.length} murid berhasil menerima program.`);
    router.refresh();
  }
  ...
  return (
    <div className="space-y-3">
      <SectionBox title="Minat Olahraga" description="Pilih minat olahraga agar program dan murid yang tampil sesuai.">
        <div className="flex flex-wrap gap-2">
          {sports.map((sport) => {
            ...
            const studentCount = students.filter((student) => student.sportId === sport.id).length;
            return (
              <button
                key={sport.id}
                className={cn(
        ...
        <SectionBox title="Pilih Murid" description="Centang murid yang akan menerima program.">
          <div className="divide-y">
            {visibleStudents.map((student) => (
              <label
                key={student.id}
                className={`grid cursor-pointer grid-cols-[auto_1fr_auto_minmax(86px,120px)] items-center gap-3 border-l-2 px-2 py-2 text-xs transition-colors hover:bg-slate-50/90 ${
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border bg-gradient-to-r from-slate-50 via-white to-emerald-50/50 px-3 py-2 shadow-sm shadow-slate-200/50">
        <p className={message.includes("berhasil") ? "text-xs text-emerald-700" : "text-xs text-red-700"}>{message}</p>
